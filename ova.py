...
def device_worker(device_id, cookie, config):
    ...
    while True:
        ...
        if config["check_interval"] > 0:
            if apk_status == "closed" or presence == "Offline":
                online_count = 0
...
def auto_rejoin():
    try:
        ...
        console.log(f"[green]Detected {len(devices)} emulators.[/green]")
...
def main_menu():
    while True:
        ...
        choice = input("Select menu (0-3): ").strip()
        ...
        if choice == "1":
            ...
            console.print("[green]Thank you! Program stopped.[/green]")
            break
        else:
